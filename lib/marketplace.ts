...
type ServiceRow = { id: string; title: string;description:string; active: boolean; canonical_services: CanonicalRow };
...
type ProviderSiteRow={headline:string;intro:string;years_experience:number|null;cover_path:string|null;theme:Provider["site"]["theme"];accent_color:string;white_label:boolean;social_links:Provider["site"]["socialLinks"]}|null;
type ProviderFaqRow={id:string;question:string;answer:string;sort_order:number;active:boolean};
type ProviderRow = {
  ...
  updated_at:string;provider_verifications:VerificationRow[];reviews:ReviewRow[];provider_service_areas:AreaRow[];provider_business_affiliations:ProviderAffiliationRow[];provider_site_settings:ProviderSiteRow;provider_faqs:ProviderFaqRow[];
};
...
function mapProvider(row:ProviderRow):Provider{const affiliation=row.provider_business_affiliations.find((item)=>item.status==="active")?.businesses;const settings=row.provider_site_settings;const provider={
  ...
  businessSlug:affiliation?.slug,businessName:affiliation?.name,isDemo:row.is_demo,canContact:!row.is_demo,avatarImage:optionalStorageUrl("avatars",row.avatar_path),avatarPath:row.avatar_path??undefined,updatedAt:row.updated_at??new Date(0).toISOString(),
  site:{headline:settings?.headline||`${row.profession} en ${row.zone}`,intro:settings?.intro||row.bio,yearsExperience:settings?.years_experience??undefined,coverImage:optionalStorageUrl("provider-work",settings?.cover_path),coverPath:settings?.cover_path??undefined,theme:settings?.theme??"tequit",accentColor:settings?.accent_color??"#254432",whiteLabel:Boolean(settings?.white_label),socialLinks:settings?.social_links??{}},
  faqs:row.provider_faqs.filter((item)=>item.active).sort((a,b)=>a.sort_order-b.sort_order).map((item)=>({id:item.id,question:item.question,answer:item.answer,sortOrder:item.sort_order,active:item.active})),
} satisfies Omit<Provider,"seo">;return{...provider,seo:providerSeoEligibility(provider)}}
...
const providerSelect=`id,slug,name,profession,bio,phone,zone,plan,status,rating,review_count,avatar_path,is_demo,updated_at,provider_services(id,title,description,active,canonical_services(id,slug,name,service_categories(name),service_aliases(alias))),provider_media(id,title,description,storage_path),provider_verifications(type,verified_at),reviews(id,customer_name,rating,comment,created_at,status,review_requests(source)),provider_service_areas(service_areas(name)),provider_business_affiliations(status,businesses(slug,name)),provider_site_settings(headline,intro,years_experience,cover_path,theme,accent_color,white_label,social_links),provider_faqs(id,question,answer,sort_order,active)`;
