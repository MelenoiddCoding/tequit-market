...
export function RequestForm({ targetProvider, targetBusiness, defaultService = "" }: RequestFormProps) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(event.currentTarget);const files=form.getAll("photos").filter((item):item is File=>item instanceof File&&item.size>0).slice(0,4);
    const payload={requestedService:String(form.get("requestedService")??""),description:String(form.get("description")??""),zone:String(form.get("zone")??""),timing:String(form.get("timing")??""),customerName:String(form.get("customerName")??""),customerPhone:String(form.get("customerPhone")??""),customerEmail:String(form.get("customerEmail")??""),website:String(form.get("website")??""),targetProvider,targetBusiness,files:files.map((file)=>({name:file.name,type:file.type,size:file.size}))};
    try { const response = await fetch("/api/leads", { method: "POST",headers:{"Content-Type":"application/json"}, body:JSON.stringify(payload) }); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? "No pudimos enviar tu solicitud.");if(files.length&&data.uploads?.length){const supabase=createClient();const confirmed:Array<{path:string;contentType:string}>=[];for(let index=0;index<Math.min(files.length,data.uploads.length);index++){const upload=data.uploads[index];const{error:uploadError}=await supabase.storage.from("upload-quarantine").uploadToSignedUrl(upload.path,upload.token,files[index],{contentType:files[index].type});if(!uploadError)confirmed.push({path:upload.path,contentType:files[index].type})}if(confirmed.length)await fetch(`/api/leads/${data.id}/media`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({uploadToken:data.uploadToken,files:confirmed})})}setSuccess(data.id); }
