...
export function ServiceManager({initialServices,entityId,kind,plan}:{initialServices:Service[];entityId:string;kind:"provider"|"business";plan:Plan}) {
  const [items, setItems] = useState<ManagedService[]>(initialServices.map((service) => ({ ...service, active: true })));
  const [error, setError] = useState("");
  async function add(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError("");
    if (plan==="free"&&items.filter((item)=>item.active).length >= 5) { setError("Llegaste al límite de 5 servicios del plan Free."); return; }
    const form = event.currentTarget; const name = String(new FormData(form).get("name") ?? "").trim();
    const response = await fetch("/api/provider/services", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({action:"add",name,kind,entityId}) });
  ...
  async function toggle(id: string) { const current=items.find((item)=>item.id===id);if(!current)return;const active=!current.active;const response=await fetch("/api/provider/services",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"toggle",id,active,kind,entityId})});if(response.ok)setItems((values)=>values.map((item)=>item.id===id?{...item,active}:item));else setError((await response.json()).error??"No pudimos actualizar el servicio.") }
  return <>
    {plan==="free"&&<div className={styles.counter}><div><strong>{items.filter((item)=>item.active).length} de 5 servicios usados</strong><p>El plan Free permite publicar hasta cinco servicios.</p></div><div className={styles.progress} aria-label={`${items.filter((item)=>item.active).length} de 5 servicios usados`}><span style={{ width: `${Math.min(items.filter((item)=>item.active).length / 5 * 100, 100)}%` }} /></div></div>}
