...
const bottomItems = [
  ...
  { href: "/login", Icon: UserRound, label: "Cuenta", matches: (path: string) => path.startsWith("/login") || path.startsWith("/registro") },
] as const;
...
export function BottomNavigation() {
  const pathname = usePathname();
  return <nav className="bottom-nav" aria-label="Navegación móvil">{bottomItems.map(({ href, Icon, label, matches }) => { const active = pathname === "/solicitar" ? false : matches(pathname); return <Link key={href} className={cn(active && "is-active")} href={href} aria-current={active ? "page" : undefined}><Icon aria-hidden /><span>{label}</span></Link>; })}</nav>;
}
...
export function PublicChrome({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const workspace = pathname.startsWith("/dashboard") || pathname.startsWith("/admin");
