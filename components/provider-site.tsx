...
import {ProviderMarketplaceLink,ProviderSiteNavigation} from "@/components/provider-site-navigation";
...
export function ProviderSite({provider,referrer}:{provider:Provider;referrer?:string}){
  ...
  return <main className={`${styles.site} ${styles[`theme_${provider.plan==="pro"?provider.site.theme:"tequit"}`]}`} style={{"--provider-accent":accent} as React.CSSProperties}>
    ...
    <ProviderSiteNavigation name={provider.name} avatar={provider.avatarImage} slug={provider.slug} proBrand={proBrand}/>
    ...
    <div className={styles.content}>
      ...
      <section id="trabajos" className={styles.section}><p className={styles.eyebrow}>Trabajo real</p><h2>Proyectos realizados</h2><PortfolioGallery portfolio={provider.portfolio}/></section>
      ...
      <section id="resenas" className={styles.section}><p className={styles.eyebrow}>Clientes anteriores</p><h2>Reseñas</h2><ReviewList reviews={provider.reviews}/></section>
      ...
      <section className={styles.finalCta}><MessageCircle aria-hidden/><h2>¿Tienes un trabajo en mente?</h2><p>Escríbele directamente a {provider.name} y cuéntale qué necesitas.</p><WhatsAppButton phone={provider.phone} message={providerMessage(provider.name,provider.profession)} label="Contactar por WhatsApp" className={styles.whatsapp} targetSlug={provider.slug} targetType="provider"/><ProviderShareTools slug={provider.slug} name={provider.name} subtitle={provider.profession} compact/></section>
    </div>
    <footer className={styles.footer}><div className={styles.footerIdentity}><BrandLogo variant="symbol"/><div><strong>{proBrand?`Sitio profesional de ${provider.name}`:`${provider.name} · Publicado en Tequit`}</strong><p>Encuentra personas y negocios en Tepic que puedan hacer el trabajo que necesitas.</p></div></div><nav aria-label="Continuar en Tequit"><ProviderMarketplaceLink slug={provider.slug} destination="search" href="/buscar">Buscar servicios</ProviderMarketplaceLink><ProviderMarketplaceLink slug={provider.slug} destination="businesses" href="/negocios">Ver negocios</ProviderMarketplaceLink><ProviderMarketplaceLink slug={provider.slug} destination="request" href="/solicitar">Publicar una necesidad</ProviderMarketplaceLink></nav></footer>
