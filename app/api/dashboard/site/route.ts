...
const schema = z.object({
  ...
  avatarPath: z.string().max(300).nullable(),
  theme: z.enum(["tequit", "claro", "oscuro", "tierra"]),
  ...
  faqs: z
    .array(
      z.object({
        ...
        answer: z.string().trim().min(12).max(600),
      }),
    )
    .max(6),
});
...
export async function POST(request: Request) {
  ...
  if (!parsed.success)
    return NextResponse.json(
      {
        error: parsed.error.issues[0]?.message ?? "Revisa los datos del sitio.",
      },
      { status: 400 },
    );
  ...
  const validPath = (path: string | null) =>
    !path || path.startsWith(`${user.id}/`);
  if (!validPath(value.coverPath) || !validPath(value.avatarPath))
    return NextResponse.json(
      { error: "Ruta de imagen inválida." },
      { status: 400 },
    );
  const plan = await getProviderPlan(value.providerId);
  const isPro = plan.entitlements.customSiteBranding;
  const settings = {
    ...
    theme: isPro ? value.theme : "tequit",
    accent_color: isPro ? value.accentColor : "#254432",
    white_label: isPro ? value.whiteLabel : false,
    social_links: Object.fromEntries(
      Object.entries(value.socialLinks).filter(([, url]) => url),
    ),
  };
  ...
  if (settingsError)
    return NextResponse.json(
      { error: "No pudimos guardar el sitio." },
      { status: 403 },
    );
  const { error: profileError } = await supabase
    .from("provider_profiles")
    .update({ bio: value.intro, avatar_path: value.avatarPath })
    .eq("id", value.providerId);
  if (profileError)
    return NextResponse.json(
  ...
  if (value.faqs.length) {
    const { error: faqError } = await supabase.from("provider_faqs").insert(
    ...
    if (faqError)
      return NextResponse.json(
        { error: "Guardamos el sitio, pero no las preguntas frecuentes." },
        { status: 500 },
      );
  }
