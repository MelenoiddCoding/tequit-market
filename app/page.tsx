...
import { BadgeCheck, BrickWall, Car, GraduationCap, Hammer, HeartPulse, Laptop, MapPin, Palette, PawPrint, Scissors, Search, Sparkles, Truck, Utensils, Wrench } from "lucide-react";
...
const iconList = [BrickWall,Wrench,Car,Truck,Sparkles,Utensils,Scissors,HeartPulse,GraduationCap,Laptop,Palette,PawPrint,Hammer];

export default async function HomePage() {
  const [{providers,businesses,services},taxonomy]=await Promise.all([getMarketplace(),getProviderTaxonomy()]);
  ...
  return <main>
    ...
    <TrustProof/>
    <Section><SiteContainer><SectionHeader eyebrow="Accesos rápidos" title="¿Qué hay que resolver?" description="Empieza por una categoría y compara opciones en Tepic." action={<Link className="btn btn-secondary" href="/buscar">Ver todos los servicios</Link>}/><AutoGrid kind="cards" className="category-row">{categories.map((category,index)=>{const Icon=iconList[index%iconList.length];return <Link key={category.slug} className="category-link" href={`/categorias/${category.slug}`}><span className="category-icon"><Icon size={22} aria-hidden/></span><strong>{category.name}</strong></Link>})}</AutoGrid></SiteContainer></Section>
