...
export default async function AccountPage({searchParams}:PageProps<"/cuenta">){
  ...
  const[{data:provider},taxonomy]=await Promise.all([supabase.from("provider_profiles").select("id,status,name,profession,phone,zone,bio,avatar_path,onboarding_step,provider_site_settings(cover_path),provider_services(title,description),provider_categories(canonical_category_id,assignment_role)").eq("owner_profile_id",session.user.id).maybeSingle(),getProviderTaxonomy()]);
  ...
  const relation=provider?.provider_site_settings as unknown as {cover_path:string|null}|null;const service=(provider?.provider_services as unknown as {title:string;description:string|null}[]|null)?.[0];const assignments=(provider?.provider_categories as unknown as {canonical_category_id:string;assignment_role:"primary"|"secondary"}[]|null)??[];
  const providerInitial=provider?{name:provider.name,profession:provider.profession,phone:provider.phone,zone:provider.zone,bio:provider.bio,service:service?.title??"",serviceDescription:service?.description??"",avatarPath:provider.avatar_path,avatarUrl:storage("avatars",provider.avatar_path),coverPath:relation?.cover_path??null,coverUrl:storage("provider-work",relation?.cover_path),step:provider.onboarding_step??1,primaryCategoryId:assignments.find(item=>item.assignment_role==="primary")?.canonical_category_id??"",secondaryCategoryIds:assignments.filter(item=>item.assignment_role==="secondary").map(item=>item.canonical_category_id)}:undefined;
