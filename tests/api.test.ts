...
describe("endpoints públicos controlados",()=>{
  ...
  it("registra navegación controlada desde un sitio de prestador",async()=>{const response=await createEvent(new Request("http://local/api/events",{method:"POST",body:JSON.stringify({type:"marketplace_nav_click",target:"juan-perez",targetType:"provider",destination:"search"})}));expect(response.status).toBe(201);expect((await response.json()).recorded).toBe(true)});
  it("rechaza destinos de navegación arbitrarios",async()=>{const response=await createEvent(new Request("http://local/api/events",{method:"POST",body:JSON.stringify({type:"marketplace_nav_click",target:"juan-perez",targetType:"provider",destination:"external-admin"})}));expect(response.status).toBe(400)});
});
